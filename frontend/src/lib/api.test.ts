  ...
  it("collapses parallel 401s into a single refresh", async () => {
    ...
    fetch.mockImplementation((url: string) => {
      ...
      return Promise.resolve(
        fetch.mock.calls.filter((call: unknown[]) => !String(call[0]).includes("refresh"))
          .length <= 2
    ...
    const refreshCalls = fetch.mock.calls.filter((call: unknown[]) =>
...
describe("error translation", () => {
  ...
  it("collects field errors from a validation response", async () => {
    ...
    try {
      ...
      expect((error as ApiError).fieldError("prompt")).toBe(
  ...
  it("reports a network failure as a readable message", async () => {
    ...
    try {
      ...
      expect((error as ApiError).message).toMatch(/could not reach chitra ai/i);
    }
  });
